...
line two` ) and will not work with special characters inside of quotes ( example: "line one line two" ) */
    ...
    if (clacType === 1) {
      ...
      month = tempDate.getMonth().toString().padStart(2, '0');
  ...
  const weekTopDate = (Variables, initFlg) => {
    ...
    if (initFlg != null) {
      ...
      for (let j = 0; j < weekDays.length; j++) {
        console.log(newDay.getDate() + '' + (j - index).toString());
        console.log(newDay.getDate() + (j - index));
        arrDay.setDate(newDay.getDate() + (j - index));
        console.log(arrDay);
        daysArr.push(arrDay.getDate().toString() + '(' + weekDays[j] + ')');
      }
      newDay.setDate(thisDay.getDate() - 7);
      console.log(daysArr);
      setThisDay(newDay);
      setDay(daysArr);
    } else {
  ...
  const [isYearFlg, setIsYearFlg] = React.useState(Constants['IsFalse']);
